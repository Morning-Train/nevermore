...
const {awaitPromises} = require("./promise");
...
class Container {
	...
	startOfWeek(date = null) {
		return (date || this.date).clone().startOf("isoWeek");
	}

	endOfWeek(date = null) {
		return (date || this.date).clone().add(1, "week").endOf("isoWeek");
	}
	...
	replaceItems(items) {
		return awaitPromises(...this.items.map(item => this.unmountItem(item, {silent: true}))).then(() => {
			return awaitPromises(items.map(item => this.mountItem(new Item(item), {silent: true})));
		});
	}
