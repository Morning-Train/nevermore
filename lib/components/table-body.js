...
class TableBody {
	...
	constructor(container) {
		...
		container.dateAtMouse = (e) => {
			...
			const minTime = moment().startOf("day").add(container.options.minTime, "hour");
			const maxTime = moment().startOf("day").add(container.options.maxTime, "hour");

			const day = Math.floor(interpolate(left, 0, 7));
			const time = interpolateDate(top, minTime, maxTime);

			return container.getDate()
				.clone()
				.startOf("isoWeek")
				.add(day, "day")
				.hour(time.hour())
