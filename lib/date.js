...
function boundedByIsoDay(date, isoDay) {
	const day = normalizeIsoDayOfWeek(date);
	return (day === isoDay) || ((day === isoDay + 1) && date.isSame(date.clone().startOf("day")));
}

function getLowerBoundIsoDay(date) {
	const day = normalizeIsoDayOfWeek(date);
	const bound = date.isSame(date.clone().startOf("day")) ? day - 1 : day;
	return bound < 0 ? 7 + bound : bound;
}

function isDayBoundary(date) {
...
function isTimeAfter(date, comparison) {
	return comparison.clone().startOf("day").add(date.diff(date.clone().startOf("day"))).isAfter(comparison);
}

function isTimeBefore(date, comparison) {
	return isTimeAfter(comparison, date);
}
