...
function spawnsInWeek(item, week) {
	if (item.repeats) {
		return item.repeatsOn(week);
	}

	return inIsoWeek(item.from, week);
}
...
function strictCollisionFilter(container, item, simulatedItem) {
	...
	for (let i = 0; i < container.items.length; i++) {
		...
		if (spawnsInWeek(item, date) && spawnsInWeek(sibling, date) && itemsOverlap(simulatedItem, sibling, date)) {
			return false;
		}
	}
}
